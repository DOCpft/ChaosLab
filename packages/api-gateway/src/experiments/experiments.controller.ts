...
export class ExperimentsController {
    ...
    async getOneExp(@Query('id') id: string, @CurrentUser() user): Promise<GetExperimentsResponseDto>{
        ...
        let result = new GetExperimentsResponseDto(); 
        if(!experiment){
            result = { statusCode: HttpStatus.NOT_FOUND, message: "This experiments is not exists" };
            return result;
        }
    ...
    async changeExperiment(@Query('id') id: string,
        ...
        if(!experiment){
            result = { statusCode: HttpStatus.BAD_GATEWAY, message: "Unable to update experiment" }
            return result
        }
